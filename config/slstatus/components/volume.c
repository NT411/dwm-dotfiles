...
volatile sig_atomic_t volume_refresh = 1;

const char *
vol_pipewire(const char *unused)
{
	static char value[32] = "n/a";
	char line[128];
	double volume;
	FILE *fp;
	int valid, status;
	...
	if (!volume_refresh)
		return value;
	/* A signal arriving during the query must request another refresh. */
	volume_refresh = 0;
	if (!(fp = popen("LC_ALL=C wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null", "r"))) {
		snprintf(value, sizeof(value), "n/a");
		return value;
	}
	valid = fgets(line, sizeof(line), fp) &&
	        sscanf(line, "Volume: %lf", &volume) == 1 &&
	        isfinite(volume) && volume >= 0 && volume <= 100;
	status = pclose(fp);
	if (valid && status == 0)
		snprintf(value, sizeof(value), "%.0f%%%s", volume * 100,
		         strstr(line, "[MUTED]") ? " muted" : "");
	else
		snprintf(value, sizeof(value), "n/a");
	return value;
}
